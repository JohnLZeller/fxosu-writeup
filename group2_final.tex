...
This file holds the bulk of the work that we've done in this project. They work can be divided into two main parts.  There are a couple of global functions and variable which are used for IPC from NetworkStatsService.js and a prototype of our XPCOM Component called FxOSUService.  A subset of the functions in this prototype are then exposed as navigator properties.  These can be be seen in FxOSUService.webidl section below.  Here are function prototypes for this file as well as brief descriptions as to what they do.
\begin{itemize}
\item \textbf{Global Functions}
...
\item \textbf{FxOSUService Functions}
\begin{lstlisting}[language=JavaScript]
FxOSUService.init()
\end{lstlisting}  
...
\end{lstlisting}
Returns a dictionary object that holds the success rate, \# of successes, and number of attempts for the last number of requests. It is used internally to determine the quality of the network for a specific group of requests.
\begin{lstlisting}[language=JavaScript]
